...
def datacard(inpath, cfgpath, dc="datacard.txt", shape="shape.h5", draw=False):
    with open(cfgpath, 'r') as f:
        cfg = yaml.full_load(f)

    systs = copy.deepcopy(cfg["mc"]["systs"])
    df_data, df_mc = open_dfs(inpath, cfg, systs)

    systs += [s+"_smooth" for s in cfg["mc"]["systs"]]
    systs += [s+"_flat" for s in cfg["mc"]["systs"]]
    create_fitinputs(df_data, df_mc, shape, dc, cfg, systs)
